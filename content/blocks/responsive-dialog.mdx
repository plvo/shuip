...
> **Wrapper elements (e.g. `<form>`):** Any element wrapping `Header/Body/Footer` inside `ResponsiveDialogContent` must carry `className="flex flex-col flex-1 min-h-0"` so it participates in the flex layout. Without it the body has no bounded height and cannot scroll.
>
> ```tsx
> <ResponsiveDialogContent>
...
<TypeTable
  type={{
    className: {
      description: 'Additional CSS classes. On mobile, px-4 py-3 padding is applied automatically.',
      type: 'string?',
    },
    children: {
      description: 'Scrollable body content. Header and footer remain anchored.',
