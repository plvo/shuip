...
<TypeTable
  type={{
    ...
    title: {
      description: 'Renders the card title (CardTitle). Return a string or any ReactNode (e.g. a link or an icon).',
    ...
    cardContent: {
      description: 'Escape hatch for the card content area (replaces fields). The title still comes from `title` in the CardTitle.',
      type: '(item: T) => ReactNode',
    },
    searchableFields: {
