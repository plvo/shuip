...
- **Structured cards**: the title comes from `titleField` (rendered as a `CardTitle`); the content area is driven by a `fields` config or fully customized with `renderCard`.
